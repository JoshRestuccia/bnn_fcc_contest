module bnn #(
    ...
    parameter int TOPOLOGY[LAYERS+1] = '{0: 784, 1: 256, 2: 256, 3: 10, default: 0},
    ...
    input  logic [              $clog2(LAYERS):0] layer_id,
    input  logic [           PARALLEL_INPUTS-1:0] input_data,
    input  logic                                  valid_in,
    output logic [PARALLEL_NEURONS[LAYERS-1]-1:0] result,
    output logic [$clog2(TOPOLOGY[LAYERS-2]+1):0] popcount         [PARALLEL_NEURONS[LAYERS-1]],
    output logic [PARALLEL_NEURONS[LAYERS-1]-1:0] valid_out,
    output logic                                  ready
);

    logic [MAX_PARALLEL_INPUTS-1:0] layer_input_data[LAYERS];
    logic                           layer_valid_in  [LAYERS];
    logic [MAX_PARALLEL_INPUTS-1:0] layer_result    [LAYERS];
    logic [MAX_PARALLEL_INPUTS-1:0] layer_valid_out [LAYERS];
    logic                           layer_wr_en     [LAYERS];
    ...
    generate
        for (i = 0; i < LAYERS; i++) begin : g_layers
            ...
            end else if (i < LAYERS - 1) begin : g_hidden_layer
                logic [$clog2(TOPOLOGY[i]+1):0] unused_popcount[PARALLEL_NEURONS[i]];
                logic unused_ready;
